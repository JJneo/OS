/*
 * ImageMaker.c
 *	디스크 이미지를 만들어주는 imageMaker.c 의 소스코드
...
int main(int argc, char* argv[])
{
	...
	// 파일 크기를 섹터 크기인 512바이트에 맞추기 위해 나머지 부분을 0x00으로 채움
	...
	printf( "[INFO] Imeage file create complete\n" );
...
// 현재위치부터 512바이트 배수 위치까지 맞추어 0x00으로 채움
int AdjustInSectorSize( int iFd, int iSourceSize )
{
	int i;
	...
	{
		printf("[INFO] File size is aligned 512Byte\n");
	}
...
// 소스파일(Source FD)의 내용을 목표 파일(Target FD)에 복사하고 그 크기를 되돌려 줌
int CopyFile(int iSourceFd, int iTargetFd )
{
	int iSourceFileSize;
	int iRead;
	...
	while( 1 )
	{
		...
		if( iRead != iWrite )
		{
			fprintf( stderr, "[ERROR] iRead != Write.. \n" );
			exit(-1);
		}
