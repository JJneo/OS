...
BOOL kWaitForACKAndPutOtherScanCode(void)
{
	...
	// ACK가 오기 전에 키보드 출력 버퍼(포트 0x60)에 키 데이터가 저장될 수 있음으로
	// 키보드에서 전달된 데이터를 최대 100개까지 수신하여 ACK를 확인
	for( j=0; j<100; j++)
	{
		// 0xFFFF만큼 루프를 수행할 시간이면 충분히 커맨드의 응답이 올 수 있음.
		// 0xFFFF루프를 수행한 이후에도 출력 버퍼(포트 0x60)이 차있지 않으면 무시하고 읽음
		for( i=0; i<0xFFFF; i++)
		{
			// 출력 버퍼(포트0x60)이 차 있으면 데이터를 읽을 수 있음
