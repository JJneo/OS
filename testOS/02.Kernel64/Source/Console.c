...
void kSetCursor(int iX, int iY)
{
	...
	// CRTC 컨트롤 어드레스 레지스터 (포트 0x3D5)에 커서의 상위 바이트를 출력.
...
int kConsolePrintString( const char* pcBuffer)
{
	...
	for(i=0; i<iLength; i++)
	{
		...
		if( pcBuffer[i] == '\n')
		{
			...
			// 현재 라인 남은 문자열 수 만큼 더해서 다음 라인으로 위치시킴
		...
		// 출력할 위차가 화면의 최대값(80*25)를 벗어났으면 스크롤 처리 ( >= 성립하자 마자 바로 그냥 올려버림 )
...
void kPrintStringXY(int iX, int iY, const char* pcString)
{
	...
	//비디오 메모리 어드레스에서 현재 출력할 위치 계산 ( 왜 책에는 CONSOLE_WIDTH 가 아니라 80일까..)
