...
BOOL kSetInterruptFlag( BOOL bEnableInterrupt )
{
	...
	// 이전의 RFLAGS 레지스터 값을 읽은 뒤 인터럽트 가능/불가 처리
	...
	// 이전 RFLAGS 레지스터의 IF비트(비트9)를 확인하여 이전의 인터럽트 상태를 반환
	if(qwRFLAGS & 0x0200)
	{
		return TRUE;
	}
...
void kCheckTotalRAMSize( void )
{
	...
	while(1)
	{
		...
		// 0x12345678을 써서 읽었을 때 문제가 없는 곳 까지를 유요한 메모리 영역으로 인정
...
// 16진수 값을 문자열로 변환 , return 값은 길이인듯.
int kHexToString( QWORD qwValue, char* pcBuffer)
{
	...
	//버퍼에 들어있는 문자열을 뒤집어서 ...256, 16, 1 의 자리 순서로 변경
...
int kDecimalToString(long lValue, char* pcBuffer)
{
	...
	//버퍼에 1의 자리부터 10, 100, 1000... 의자리 순서로 숫자 삽입
	for(; lValue>0; i++ )
	{
	...
	// 버퍼에 들어있는 문자열을 뒤집어서... 1000, 100, 10, 1 의 자리 순서로 변경
	if( pcBuffer[0] == '-' )
	{
...
int kVSPrintf( char* pcBuffer, const char* pcFormatString, va_list ap )
{
	...
	for( i=0; i<iFormatLength; i++ )
	{
		if(pcFormatString[i] == '%' )
		{
			...
			case 's':
				...
				// 문자 출력
			case 'c':
				// 가변 인자에 들어있는 파라미터를 문자 타입으로 변환하여 출력 버퍼에 복사하고 버퍼의 인덱스를 1만큼 이동
			...
			case 'd':
			case 'i':
				//가변 인자에 들어있는 파라미터를 정수 타입으로 변환하여 출력 버퍼에 복사하고 출력한 길이만큼 버퍼의 인덱스로 이동
			...
			case 'Q':
			case 'p':
				//가변 인자에 들어있는 파라미터를 QWORD 타입으로 변환하여 출력 버퍼에 복사하고 출력한 길이만큼 버퍼의 인덱스를 이동
				qwValue = (QWORD) (va_arg( ap, QWORD) );
				iBufferIndex += kIToA(qwValue, pcBuffer+iBufferIndex, 16);
				break;

				// 위에 해당하지 않으면 문자를 그대로 출력하고 버퍼의 인덱스르 1 만큼 이동
			default:
	...
	// NULL을 추가해 완전한 문자열로 만들고 출력한 물자의 길이를 반환
