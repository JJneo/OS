...
TCB* kCreateTask( QWORD qwFlags, QWORD qwEntryPointAddress )
{
	...
	pvStackAddress = (void*) (TASK_STACKPOOLADDRESS + (TASK_STACKSIZE * ( pstTask->stNode.qwID & 0xFFFFFFFF) ) );
...
BOOL kScheduleInInterrupt( void )
{
	...
	// 전환해서 실행할 태스크를 RunningTask로 설정하고 콘텍스트를 IST에 복사해서 자동으로 태스크 전환이 일어나도록 함.
	gs_stScheduler.pstRunningTask = pstNextTask;
